...
model_urls = {('unet', 'R231'): ('../input/unet-r231/unet_r231-d5d2fc3d.pth', 3),
              ('unet', 'LTRCLobes'): (
...
def get_model(modeltype, modelname):
    ...
    state_dict = state_dict = torch.hub.load_state_dict(model_url, progress=True, map_location=torch.device('cpu'))
    if modeltype == 'unet':
